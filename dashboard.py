...
def gerar_pdf_basico(
    ...
    """
    Gera um PDF com métricas, tabelas e gráficos Plotly.
    
    Args:
        ... (dados anteriores)
        fig_classes: Gráfico Plotly de distribuição por classe
        fig_rf_rv: Gráfico Plotly de Renda Fixa vs Variável
        fig_nac_int: Gráfico Plotly de Nacional vs Internacional
        fig_top10: Gráfico Plotly do Top 10
    """
    ...
    def truncar_produto(texto, max_len=35):
        """Trunca apenas se necessário."""
    ...
    # ── 1. Título
    elementos.append(Paragraph("📊 Relatório — Carteira de Investimentos", titulo_style))
    ...
    elementos.append(Paragraph("💰 Patrimônio Total", secao_style))
    
    rf_total = df_rf_rv[df_rf_rv['Tipo'] == 'Renda Fixa']['Valor'].values[0]
    rv_total = df_rf_rv[df_rf_rv['Tipo'] == 'Renda Variável']['Valor'].values[0]
    
    metricas_list = [
        ('Patrimônio Líquido Total', patrimonio_total, 100.0),
        ('Renda Fixa', rf_total, rf_total/patrimonio_total*100),
        ('Renda Variável', rv_total, rv_total/patrimonio_total*100)
    ]
    
    metricas_ordenadas = [metricas_list[0]] + sorted(metricas_list[1:], key=lambda x: x[2], reverse=True)
    
    dados_metricas = [['Métrica', 'Valor', 'Percentual']]
    
    for metrica, valor, percentual in metricas_ordenadas:
    ...
    tabela_metricas = Table(dados_metricas, colWidths=[3.2*inch, 1.8*inch, 0.9*inch])
    ...
    # ── 5. Distribuição por classe (tabela ordenada)
    elementos.append(Paragraph("🥧 Alocação por Classe de Ativo", secao_style))
    
    df_classes_ordenado = df_classes.sort_values('Valor', ascending=False).reset_index(drop=True)
    
    dados_classes = [['Classe', 'Valor (R$)', 'Percentual']]
    
    for _, row in df_classes_ordenado.iterrows():
        percentual = row['Valor'] / patrimonio_total * 100
    ...
    tabela_classes = Table(dados_classes, colWidths=[2.5*inch, 1.8*inch, 0.9*inch])
    
    tabela_classes.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#636EFA')),
        ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
        ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'),
        ('FONTSIZE', (0, 0), (-1, 0), 11),
    ...
    elementos.append(tabela_classes)
    elementos.append(Spacer(1, 0.3*inch))
    
    # ── 7. Tabela do Top 10 (resumida)
    df_top10_exp = df_top10.copy()
    df_top10_exp['Percentual'] = (df_top10_exp['Valor'] / patrimonio_total * 100).round(2)
    
    dados_top10 = [['#', 'Produto', 'Classe', 'Valor (R$)', '%']]
    
    for i, (_, row) in enumerate(df_top10_exp.iterrows(), start=1):
        produto_truncado = truncar_produto(str(row['Produto']), max_len=38)
    ...
    tabela_top10 = Table(
        dados_top10,
        colWidths=[0.35*inch, 2.6*inch, 1.1*inch, 1.3*inch, 0.65*inch]
    )
    
    tabela_top10.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#636EFA')),
        ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
        ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'),
        ('FONTSIZE', (0, 0), (-1, 0), 10),
